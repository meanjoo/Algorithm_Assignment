...
# 17. Color Length
생각하는 데 진짜진짜진짜 오래 걸렸다. 이틀 정도 수시로 이 문제만 생각했다.  
1번부터 17번 중에 가장 오래 생각한 것 같다. 그래서 당연히 삽질도 겁나게 많이 했다.

설명은 대충 코드에 있는 주석을 참고하고,  
...
지금 생각해보니까 중앙값이 2개일 때 굳이 걔네 둘의 평균으로 안 하고 둘 중 아무거나 골라도 되는 게 어차피 그 사이의 모든 값이 최적해네 허허
...
만약 k를 m까지 만들면 dp[0][n-1][m]이 바로 답일 텐데, 나는 m까지 안 만들고 m-1까지만 만들어서 반복을 한 번 더 돌렸다.  
왜 이렇게 짰는가에 대해서는 딱히 이유는 없고 그때 그렇게 짜고 싶었나봄

뭔가 시간복잡도 더 줄이는 방법이 있을 거 같은데 할 게 많아서 생각해볼 시간이 없네..  
아무리 생각해도 4중 반복문은 좀 아닌 거 같은데 음
...
이렇게만 갱신하도록 되어 있는데 for문 안에서 dp[i][j][cnt] = min(dp[i][j][cnt], dp[i][k][cnt-1]+dp[k+1][j][1])도 해줘야 하는 거 아닌가?? 왜 답이 맞지?
앞 쪽에서부터만 한 개씩 설치하게 해봤는데 뒤에서도 해봐야하는 거 아닌가? 안 해도 되나?
...
채점 말고 예제 테케 넣었을 때 프림의 값 이상하게 나왔었는데 그 이유가 무방향 그래픈데 한 방향만 간선을 추가해줬었기 때문이다.  
MST가 연결된 비방향성 그래프에서 만들어질 수 있는 건데 크루스칼에서 인접 그래프를 안 쓰니까 처음에 프림에서 뭐가 잘못된지 몰랐었다. ㅋㅋ..  
그냥 구현 문제였다.

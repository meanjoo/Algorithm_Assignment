...
다 끝나고 다시 보는데 뭔가 94번째 줄이 잘못된 것 같다? 마지막 칸은 k번 이상인 건데 이게 최솟값이 무조건 내 앞에거가 아니라 min(나, 내 앞)이어야 할 거 같은데 그래서 path도 이에 맞게 조정해야 하는 거 같다. 만약 k번 이상 지나가는 최소 구하는 거였으면 잘못된 답이 
